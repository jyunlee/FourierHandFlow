...
class LEAP(nn.Module):
    def __init__(self,
        ...
        if leap_path is not None:
            ...
            self.model = OurLEAPModel_StructureOnly_NoCycle_ShapeNet.load_from_file(leap_path)
        ...
        self.shape_dirs = torch.tensor(np.asarray(shape_dirs), dtype=dtype)
        self.pose_dirs = torch.tensor(pose_dirs, dtype=dtype)
        self.joint_regressor = torch.tensor(joint_regressor.todense(), dtype=dtype)
        self.kintree_table = torch.tensor(kintree_table, dtype=torch.int32)
        self.weights = torch.tensor(weights, dtype=dtype)

        # fix left mano shape
        if self.model_type == 'mano' and 'LEFT' in bm_path:
            right_bm_path = bm_path.replace('LEFT', 'RIGHT')

            with open(right_bm_path, 'rb') as right_smpl_file:
                right_smpl_model = pickle.load(right_smpl_file, encoding='latin1')

            right_shape_dirs = torch.tensor(np.asarray(right_smpl_model['shapedirs']), dtype=dtype)

            if torch.sum(torch.abs(self.shape_dirs[:, 0, :] - right_shape_dirs[:, 0, :])) < 1:
                self.shape_dirs[:, 0, :] *= -1
    ...
    def get_num_joints(bm_path):
        model_type = LEAP.load_smpl_model(bm_path)[1]
    ...
    def get_parent_mapping(model_type):
        ...
        mano_mappings = [-1, 0, 1, 2, 0, 4, 5, 0, 7, 8, 0, 10, 11, 0, 13, 14]
